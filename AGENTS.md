...
Astro Composer is an Obsidian plugin that turns notes into posts and pages for Astro blogs with automated content management features. It provides a bridge between Obsidian notes and Astro blog content, enabling seamless content creation and management workflows. The plugin is part of the Vault CMS project and supports multiple content types with customizable templates and folder patterns. Key features include automated post creation with title dialogs, property standardization, draft management, internal link conversion, and configurable workflows for both file-based and folder-based content structures.

### Important Project-Specific Details

- **Project Type**: Production Obsidian plugin
- **Purpose**: Bridge between Obsidian notes and Astro blog content
- **Status**: Active development (version 0.9.3)
- **Key Features**: Content type management, automated post creation, property standardization, internal link conversion, draft management, ribbon icons, help button replacement
- **Architecture**: Modular structure with separate files for commands, UI components, utilities, and settings
- **Platform Support**: Works on both desktop and mobile (with some desktop-only features like ribbon icons and help button replacement)

### Maintenance Tasks

- Keep README.md updated with new features
- Maintain compatibility with latest Obsidian API (especially SettingsGroup since 1.11.0)
- Update content type migration logic when needed
- Keep `.agents/` folder in sync with reference repos
- Test on both desktop and mobile platforms
- Monitor and update for breaking changes in Obsidian API
...
- **UI**: Uses custom modals (TitleModal, CommandPickerModal, IconPickerModal, ConfirmModal, MigrationModal)
- **Settings groups**: Uses SettingsGroup with backward compatibility for API < 1.11.0
- **Content types**: Support wildcard patterns (e.g., `docs/*`, `docs/*/*`) for folder depth matching
- **Migration**: Handles legacy posts/pages settings and converts them to unified content types

### Project-Specific References

- `.ref/plugins/obsidian-ui-tweaker/` - Reference for SettingsGroup implementation pattern with version checking and backward compatibility
...
- `src/main.ts` - Main plugin file containing AstroComposerPlugin class and all core logic
- `src/ui/settings-tab.ts` - Settings tab implementation with content type management
- `src/ui/title-modal.ts` - Modal for entering post titles when creating new files
- `src/utils/settings-compat.ts` - Settings compatibility utility for backward-compatible SettingGroup usage
- `src/utils/file-operations.ts` - File creation and manipulation utilities
- `src/utils/template-parsing.ts` - Template parsing and variable substitution
- `src/utils/link-conversion.ts` - Internal link conversion utilities
- `src/utils/path-matching.ts` - Folder pattern matching with wildcard support
- `src/utils/heading-link-generator.ts` - Heading link generation utilities
- `src/commands/index.ts` - Command registration and handlers
- `src/settings.ts` - Default settings and constants
- `src/types.ts` - TypeScript type definitions
- `manifest.json` - Plugin metadata
- `package.json` - Dependencies and build scripts
- `README.md` - Project documentation
- `AGENTS.md` - This file (project-specific AI agent instructions)
- `.agents/` - General-purpose development guides

### Development Notes

- This is a production plugin for managing Astro blog content from Obsidian
- The SettingsGroup implementation demonstrates backward compatibility patterns for API 1.11.0+
- Reference UI Tweaker (`.ref/plugins/obsidian-ui-tweaker/`) for production examples of SettingsGroup usage
- The first settings group doesn't have a heading (following UI Tweaker pattern)
- Additional groups should only be created if settings naturally group together
- Content types support wildcard patterns (e.g., `docs/*`, `docs/*/*`) for folder depth matching
- Migration system handles legacy posts/pages settings and converts them to unified content types
- Ribbon icons and help button replacement are desktop-only features (not available on mobile)
- The plugin uses a debounce mechanism to prevent multiple modals from rapid file creations
- Content type matching uses pattern specificity (most specific pattern wins) to handle conflicts
